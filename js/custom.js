...
$(document).ready(function()
{
	...
	$(".category_ddm").on('click', function(){
		$(".category_menu").toggle();
	});

	$(".theme_menu").css('display','none');
	$(".theme_ddm").on('click', function(){
		$(".theme_menu").toggle();
	});
	...
	*/
	...
	function initMsgIcos()
	{
		// Handle Favorites
		var items = document.getElementsByClassName('message_ico');
		for(var x = 0; x < items.length; x++)
		{
			var item = items[x];
			item.addEventListener('click', function(fn)
	...
	*/
	...
	15. Init Trends_new Slider

	*/

	function newTrendsSlider()
	{
		if($('.newTrends_slider').length)
		{
			var newTrendsSliders = $('.newTrends_slider');
			newTrendsSliders.each(function()
			{
				var newTrendsSlider = $(this);

				initNTSlider(newTrendsSlider);
			})	
		}
	}

	function initNTSlider(bss)
	{
		var newTrendsSlider = bss;

		newTrendsSlider.slick(
		{
			rows:2,
			infinite:true,
			slidesToShow:4,
